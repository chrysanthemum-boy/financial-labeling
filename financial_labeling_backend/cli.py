...
Path(FINANCIAL_LABELING_HOME).mkdir(parents=True, exist_ok=True)
env.bool("DEBUG", False)
os.environ["STANDALONE"] = "True"
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "config.settings.production")
os.environ.setdefault("DATABASE_URL", os.path.join(f"sqlite:///{FINANCIAL_LABELING_HOME}", "db.sqlite3"))
os.environ.setdefault("MEDIA_ROOT", os.path.join(FINANCIAL_LABELING_HOME, "media"))
base = os.path.abspath(os.path.dirname(__file__))
sys.path.append(base)
parser = argparse.ArgumentParser(description="financial_labeling, "
                                             "financial text annotation for machine learning practitioners.")
